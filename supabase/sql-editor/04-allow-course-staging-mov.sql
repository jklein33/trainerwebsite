...
begin;
do $$ begin
  if not exists(select 1 from storage.buckets where id='course-staging-media' and public=false) then
    raise exception 'Expected private course-staging-media bucket was not found. Check the target project.';
  end if;
end $$;
update storage.buckets
set allowed_mime_types = array_append(allowed_mime_types, 'video/quicktime')
where id='course-staging-media' and allowed_mime_types is not null
